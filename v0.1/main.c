	 ...
	 if(deserializePacket(rx_buf, rlen, &rxpckt) || validate_packet(&rxpckt)){
		      ...
		      if(rxpckt.destId==0xFF){
		    	  ...
		    	  if(peeralreadyexist){ //If Peer Exist, Update the time by Hal_ticks, which is in milli's ;
		    		  ...
		    		  nm.Nodes[i].lastseen=HAL_GetTick();
		    		  nm.Nodes[i].Node_ID=rxpckt.sourId;
		    		  nm.Nodes[i].RSSI=LoRa_getRSSI(&myLoRa);
		    		  nm.NoofNodesPresent++;
		    	  }
		      }
...
int txq_enque_unfrag(uint8_t destId, uint8_t * payload, uint8_t plen){
	...
	pkt.destId= destId;
	pkt.msgId= alloc_msgid(destId);
	pkt.control= make_control(0,LOCAL_WINDOW,CTL_TYPE_DATA);
	pkt.pLen=plen;
	memcpy(pkt.data, payload, plen);

	temp->len = serializePacket(&pkt, temp->ser, sizeof(temp->ser));
...
void txq_try_send(void){
	...
	for(int i=txq_tail; i!=txq_head && out < LOCAL_WINDOW;	 i=(i+1)%TXQ_SIZE){
		...
		if(transmit_frame_with_byte_stuffing(e->ser, e->len)){
			...
			e->retries=0;
			out++;
			printf("Tx'ed msg of Id %d to %c ; len =%u\r\n");
		}else{
...
// Update node table and detect duplicate msgIds
// Returns: 1 = duplicate msg, 0 = new message
uint8_t nm_update_and_check(uint8_t sourId, uint8_t msgId, int rssi, uint8_t isBroadcast)
{
    uint32_t now = HAL_GetTick();
    // Search for existing node
    for (uint8_t i = 0; i < nm.NoofNodesPresent; i++) {
        if (nm.Nodes[i].Node_ID == sourId) {
            nm.Nodes[i].lastseen = now;
    ...
    if (nm.NoofNodesPresent < 10) {
        ...
        if (isBroadcast) {
            printf("New broadcast node discovered: %c\n", sourId);
        } else {
            printf("New unicast node discovered: %c\n", sourId);
        }
        return 0; // First message from new node = new
    }

    // Table full
    printf("Node table full, cannot track node: %c\n", sourId);
    return 1; // Treat as seen to avoid processing
}
...
void handle_unfrag_data_packet(const Packet* p)
{
	...
	uint8_t dup = nm_update_and_check(p->sourId, p->msgId, LoRa_getRSSI(&myLoRa), isBcast);
	// For broadcast: process but do not ACK to avoid storms
	if (is_broadcast(p->destId)) {

		if (!dup) {
			// process data
			printf("BCAST from %c: %.*s RSSI=%d\n", p->sourId, p->pLen, (char*)p->data, LoRa_getRSSI(&myLoRa));
		} else {
	...
	// Unicast: handle duplicates and ACK

	if (!dup) {
		// process new data
		printf("DATA from %c: %.*s RSSI=%d\n", p->sourId, p->pLen, (char*)p->data, LoRa_getRSSI(&myLoRa));
	} else {
		printf("Duplicate DATA from %c msgId=%u\n", p->sourId, p->msgId);
	}
	// Always ACK unicast, including duplicates
	send_ack(p->sourId, p->msgId);
}
