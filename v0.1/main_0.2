...
void nm_print_nodes(void)
{
    ...
    for (uint8_t i = 0; i < nm.NoofNodesPresent; i++) {
        ...
        uint32_t age_ms = now - n->lastseen;

        printf("Node %c | RSSI: %d dBm | last seen: %lu ms ago | nextMsgId: %u | recentMsgIds: ",
               n->Node_ID, n->RSSI, (unsigned long)age_ms, n->nextMsgId);
...
void handle_uart_cmd(const UARTCmd* ucmd) {
	if (strcmp(ucmd->type, "SEND") == 0) {
		...
		int ret = txq_enque_unfrag(dest, (uint8_t*)ucmd->data, msglen);
...
void uart_send_peerlastseen(void) {
    ...
    for(uint8_t i=0;i<nm.NoofNodesPresent;i++) {
        printf("%c(lastseen:%ds)%s", nm.Nodes[i].Node_ID,
...
// Setup LoRa pins & SPI handler specific to your board!
  ...
  while (1)
  {
	  ...
	  uint32_t now = HAL_GetTick();
	  if ((now - lastPruneTime) > 10000) { // 10 seconds
